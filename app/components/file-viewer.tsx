...
interface File {
  file_id: string; // Assuming file_id is a string
  filename: string; // Assuming filename is a string
  status: string; // Assuming status is a string
}

const FileViewer = () => {
  const [files, setFiles] = useState<File[]>([]); // Specify the type for files

  // Function to fetch files from the server
  const fetchFiles = async () => {
    const resp = await fetch("/api/assistants/files", { method: "GET" });
    const data = await resp.json();
    setFiles(data); // Update state with fetched files
  };

  useEffect(() => {
    fetchFiles(); // Call to fetch initial files
    const interval = setInterval(fetchFiles, 1000); // Fetch files every second
  ...
  const handleFileUpload = async (
    ...
    data.append("file", event.target.files[0]); // Append the first selected file
          ...
          files.map((file) => (
            <div key={file.file_id} className={styles.fileEntry}>
              ...
              <span onClick={() => handleFileDelete(file.file_id)}>
                <TrashIcon /> {/* Clickable icon to delete the file */}
              </span>
            </div>
          ))
        )}
